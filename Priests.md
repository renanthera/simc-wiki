...
SimC has an option called `priest.ignore_healing` was added. This option allows you to ignore healing events that are generated by damage spells, namely Vampiric Touch, Devouring Plague, and Mindgames. This option has focus on DPS-only simulations, in which Healing events are not important to keep track of. In these simulations, using this option will reduce the total amount of time required to finish them (very slightly).

### Example Performance Changes
- `priest.ignore_healing=1`:
  - 3002138 Sim seconds
  - 21 CPU seconds
  - 2.8845 Physical seconds
- `priest.ignore_healing=0` (Default):
  - 3002140 Sim seconds
  - 21.6562 CPU seconds
  - 3.1227 Physical seconds
