...
* **shadowlands.soleahs_secret_technique_type** (scope: global/player; default: haste) The stat buff you gain from the Soleahs Secret Technique trinket. This is the self bonus only, not the ally bonus. Options are `haste`, `crit`, `mastery`, or `versatility`. The option can also be used in player-scope; if defined, it will override the global default value.
