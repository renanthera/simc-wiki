...
A number of azerite powers and items have characteristics that can be customized through expansion specific options. All of the following options are in the sim scope (i.e., global option that affects all actors in the profile). The following options are currently implemented.
  ...
  * **bfa.undulating_tides_lockout_chance** (scope: global; default 0.0) If you fall below 50% hp, undulating tides will give you a shield and give you a debuff that prevents any damage procs for 45s. If this option is set, every 60s a check will be a made to see if there is a chance equal to the setting that the debuff will be triggered. Accepts values from 0.0 to 1.0.  
You can use the option **bfa.undulating_tides_lockout_timer** (scope: global; default 60; minimum 1) to set the frequency in seconds in which the check is made.
